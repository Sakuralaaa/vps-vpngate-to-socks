...
proxy_auth_lock = threading.RLock()
proxy_auth_username = ""
proxy_auth_password = ""

def set_proxy_auth(username: str = "", password: str = "") -> None:
    global proxy_auth_username, proxy_auth_password
    with proxy_auth_lock:
        proxy_auth_username = username or ""
        proxy_auth_password = password or ""

def proxy_auth_enabled() -> bool:
    with proxy_auth_lock:
        return bool(proxy_auth_username and proxy_auth_password)

def credentials_match(username: str, password: str) -> bool:
    with proxy_auth_lock:
        return username == proxy_auth_username and password == proxy_auth_password
...
def http_proxy_auth_ok(lines: list[str]) -> bool:
    ...
    for line in lines[1:]:
        ...
        return bool(sep) and credentials_match(username, password)
    return False
...
def http_client(client: socket.socket, first_byte: bytes) -> None:
    ...
    try:
        ...
        if not http_proxy_auth_ok(lines):
            send_http_proxy_auth_required(client)
            return
        ...
        headers = [line for line in lines[1:] if not line.lower().startswith(("proxy-connection:", "proxy-authorization:", "connection:"))]
