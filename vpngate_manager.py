...
def host_from_request(headers: Any) -> str:
    if PUBLIC_HOST:
        return normalize_host(PUBLIC_HOST)
    host_header = headers.get("Host", "") if headers else ""
    return normalize_host(host_header)

def request_origin(headers: Any) -> str:
    proto = headers.get("X-Forwarded-Proto", "http") if headers else "http"
    if PUBLIC_HOST:
        host_value = PUBLIC_HOST
        if "://" in host_value:
        ...
        if ":" not in host_value.strip("[]"):
            host_value = f"{host_value}:{UI_PORT}"
        return f"{proto}://{host_value}"
    host_header = headers.get("Host", "") if headers else ""
    if host_header:
        return f"{proto}://{host_header}"
    return f"{proto}://127.0.0.1:{UI_PORT}"
...
def load_ui_config() -> dict[str, Any]:
    with lock:
        ...
        if auth_file.exists():
            try:
                ...
                for key, val in data.items():
                    config[key] = val
                for key in ["proxy_port", "proxy_username", "proxy_password", "subscription_token", "public_host", "routing_mode", "force_country", "routing_ip_type", "connection_enabled", "fixed_node_id"]:
                    if key not in data:
                        updated = True
            except Exception:
                pass
        ...
        if not config.get("subscription_token"):
            config["subscription_token"] = generate_subscription_token()
            updated = True

        if PUBLIC_HOST and config.get("public_host") != PUBLIC_HOST:
  ...
  <div class="btn-group">
    ...
    <div class="dropdown">
      ...
      <div id="admin_dropdown" class="dropdown-content">
        ...
        <a href="javascript:void(0)" onclick="openProxyAccessModal()">
          <svg xmlns="http://www.w3.org/2000/svg" style="width:14px; height:14px;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 11c0-1.657 1.79-3 4-3s4 1.343 4 3-1.79 3-4 3-4-1.343-4-3z" /><path stroke-linecap="round" stroke-linejoin="round" d="M4 11c0-1.657 1.79-3 4-3 1.147 0 2.181.362 2.91.942M4 11c0 1.657 1.79 3 4 3 1.147 0 2.181-.362 2.91-.942M4 11v5c0 1.657 1.79 3 4 3 1.147 0 2.181-.362 2.91-.942M12 11v5c0 1.657 1.79 3 4 3s4-1.343 4-3v-5" /></svg>
          Proxy / Clash
        </a>
...
<main>
  ...
  <!-- Proxy Access Modal -->
  <div id="proxy_access_modal" class="modal">
    <div class="modal-content" style="max-width: 560px;">
      ...
      <form id="proxy_access_form" onsubmit="saveProxyAccess(event)">
        <div class="form-group" style="margin-bottom: 12px;">
          <label class="form-label" for="proxy_access_username">代理账号</label>
          <input type="text" id="proxy_access_username" class="input-field" required>
        </div>
        <div class="form-group" style="margin-bottom: 16px;">
          <label class="form-label" for="proxy_access_password">代理密码</label>
          <input type="text" id="proxy_access_password" class="input-field" required>
        </div>
        <div style="display: flex; gap: 12px; justify-content: flex-end; margin-bottom: 18px;">
          <button type="submit" id="proxy_access_submit_btn" class="btn-primary" style="height: 38px; padding: 0 18px; font-weight: 600; border-radius: 8px;">保存代理账号</button>
        </div>
      </form>
      ...
      <div style="border-top: 1px dashed rgba(190,24,93,0.18); padding-top: 16px;">
        ...
        <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 12px; flex-wrap: wrap;">
          <button type="button" onclick="copyClashSubscription()" style="height: 36px; padding: 0 14px; font-weight: 600; border-radius: 8px; border: 1px solid var(--border-color); background: transparent; color: var(--text-primary); cursor: pointer;">复制链接</button>
          <button type="button" onclick="resetSubscriptionToken()" style="height: 36px; padding: 0 14px; font-weight: 600; border-radius: 8px; border: 1px solid rgba(244,63,94,0.3); background: rgba(244,63,94,0.08); color: var(--danger); cursor: pointer;">重置订阅 Token</button>
        </div>
      </div>
    </div>
  </div>
...
function openProxyAccessModal() {
  $("proxy_access_error").style.display = "none";
  $("proxy_access_success").style.display = "none";
  $("proxy_access_form").reset();
  if (state) {
    $("proxy_access_username").value = state.proxy_username || "";
    $("proxy_access_password").value = state.proxy_password || "";
    $("proxy_access_url").textContent = buildProxyDisplayUrl();
    $("clash_subscription_url").value = state.clash_subscription_url || `${window.location.origin}${state.clash_subscription_path || ""}`;
  }
  $("proxy_access_modal").style.display = "flex";
  $("admin_dropdown").style.display = "none";
}

function closeProxyAccessModal() {
  $("proxy_access_modal").style.display = "none";
}

async function saveProxyAccess(e) {
  e.preventDefault();
  const errorDivEl = $("proxy_access_error");
  const successDiv = $("proxy_access_success");
  const submitBtn = $("proxy_access_submit_btn");
  errorDivEl.style.display = "none";
  successDiv.style.display = "none";

  const proxy_username = $("proxy_access_username").value.trim();
  const proxy_password = $("proxy_access_password").value.trim();
  if (!proxy_username || !proxy_password) {
    errorDivEl.textContent = "代理账号和代理密码不能为空";
    errorDivEl.style.display = "block";
    return;
  }
  if (proxy_username.includes(":")) {
    errorDivEl.textContent = "代理账号不能包含冒号";
    errorDivEl.style.display = "block";
    return;
  }
  ...
  submitBtn.textContent = "正在保存...";
  try {
    const res = await fetch("./api/update_proxy_credentials", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ proxy_username, proxy_password })
    });
    const data = await res.json();
    if (res.ok && data.ok) {
      successDiv.textContent = "代理账号密码已保存并立即生效";
      successDiv.style.display = "block";
      if (!state) state = {};
      state.proxy_username = proxy_username;
      state.proxy_password = proxy_password;
      $("proxy_access_url").textContent = buildProxyDisplayUrl();
      setTimeout(load, 500);
    } else {
      errorDivEl.textContent = data.error || "保存失败";
      errorDivEl.style.display = "block";
    }
  } catch (err) {
    errorDivEl.textContent = "连接服务器失败，请稍后重试";
    ...
    submitBtn.disabled = false;
    submitBtn.textContent = "保存代理账号";
  }
}

async function resetSubscriptionToken() {
  const errorDivEl = $("proxy_access_error");
  const successDiv = $("proxy_access_success");
  errorDivEl.style.display = "none";
  successDiv.style.display = "none";
  try {
    const res = await fetch("./api/reset_subscription_token", { method: "POST" });
    const data = await res.json();
    if (res.ok && data.ok) {
      if (!state) state = {};
      state.subscription_token = data.subscription_token;
      state.clash_subscription_url = data.clash_subscription_url;
      $("clash_subscription_url").value = data.clash_subscription_url;
      successDiv.textContent = "订阅 Token 已重置，旧链接已失效";
      successDiv.style.display = "block";
    } else {
      errorDivEl.textContent = data.error || "重置失败";
      errorDivEl.style.display = "block";
    }
  } catch (err) {
    errorDivEl.textContent = "连接服务器失败，请稍后重试";
...
async function copyClashSubscription() {
  const input = $("clash_subscription_url");
  input.select();
  try {
...
def build_clash_subscription(headers: Any) -> str:
    ...
    return "\n".join([
        ...
        "  - name: Proxy",
        "    type: select",
        f"    proxies: [{yaml_quote(name)}]",
        "rules:",
        "  - MATCH,Proxy",
        "",
    ])

def subscription_url_for_request(headers: Any, token: str) -> str:
    return f"{request_origin(headers)}/sub/{token}"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if raw_path.startswith("/sub/"):
            token = raw_path.removeprefix("/sub/").strip("/")
            ui_cfg = load_ui_config()
            if token and token == ui_cfg.get("subscription_token", ""):
                self.send_bytes(build_clash_subscription(self.headers).encode("utf-8"), "text/yaml; charset=utf-8")
            else:
        ...
        if effective_path in ("/", "/index.html"):
            ...
            state_data["clash_subscription_url"] = subscription_url_for_request(self.headers, state_data.get("subscription_token", ""))
            self.send_json({"nodes": stripped_nodes, "state": state_data})
    ...
    def do_POST(self) -> None:
        ...
        elif effective_path == "/api/update_proxy_credentials":
            try:
                length = parse_int(self.headers.get("Content-Length"))
                payload = json.loads(self.rfile.read(length).decode("utf-8") or "{}")
                ...
                if not proxy_username or not proxy_password:
                    self.send_json({"ok": False, "error": "代理账号和代理密码不能为空"}, HTTPStatus.BAD_REQUEST)
                    return
                if ":" in proxy_username:
                    self.send_json({"ok": False, "error": "代理账号不能包含冒号"}, HTTPStatus.BAD_REQUEST)
                    return
                ...
                with lock:
                    DATA_DIR.mkdir(exist_ok=True, parents=True)
                    auth_file.write_text(json.dumps(ui_cfg, ensure_ascii=False, indent=2), encoding="utf-8")
                proxy_server.set_proxy_auth(proxy_username, proxy_password)

                self.send_json({"ok": True, "message": "代理账号密码已更新并立即生效"})
            except Exception as exc:
                self.send_json({"ok": False, "error": str(exc)}, HTTPStatus.INTERNAL_SERVER_ERROR)
            return

        elif effective_path == "/api/reset_subscription_token":
            try:
                ui_cfg = load_ui_config()
                ui_cfg["subscription_token"] = generate_subscription_token()
                auth_file = DATA_DIR / "ui_auth.json"
                with lock:
                    DATA_DIR.mkdir(exist_ok=True, parents=True)
                    auth_file.write_text(json.dumps(ui_cfg, ensure_ascii=False, indent=2), encoding="utf-8")
                self.send_json({
                    "ok": True,
                    "subscription_token": ui_cfg["subscription_token"],
                    "clash_subscription_url": subscription_url_for_request(self.headers, ui_cfg["subscription_token"]),
                })
            except Exception as exc:
                self.send_json({"ok": False, "error": str(exc)}, HTTPStatus.INTERNAL_SERVER_ERROR)
            return
...
def main() -> None:
    ...
    ui_cfg = load_ui_config()
    proxy_username = str(ui_cfg.get("proxy_username", "proxy"))
    proxy_password = str(ui_cfg.get("proxy_password", ""))
    threading.Thread(
        target=proxy_server.start_proxy_server,
        args=(LOCAL_PROXY_HOST, LOCAL_PROXY_PORT, proxy_username, proxy_password),
        daemon=True,
    ).start()
